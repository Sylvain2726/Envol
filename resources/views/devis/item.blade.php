<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center">
            <h1 class="text-3xl md:text-5xl py-4">{{ __('Les items du devis numéro '. $devi->id) }}</h1>
            ...
            <tbody>
                @foreach ($items as $item)
                    <tr id="ligne-{{ $item->id }}" class=" ">
                        ...
                        <td class="px-5 py-3 text-sm bg-white border-b border-gray-200">
                            <p class="text-gray-900 whitespace-no-wrap">{{ $item->VPrice }}</p>
                        </td>
                        <td class="px-5 py-3 text-sm bg-white border-b border-gray-200">
                            <p class="text-gray-900 whitespace-no-wrap">{{ $item->quantite}}
                            </p>
                        </td>
                        ...
                        <td class="px-5 py-3 text-sm bg-white border-b border-gray-200">
                            <form id="supprimer-{{ $item->id }}" method="post"
                                action="{{ route('itemDevis.destroy' , ['itemDevi' => $item->id])}}"
                                >

                                @method('DELETE')
                                @csrf
                                <button data-modal-target="popup-modal-{{ $item->id }}"
                                    data-modal-toggle="popup-modal-{{ $item->id }}" type="button"
                        ...
                        <td class="px-5 py-3 text-sm bg-white border-b border-gray-200">
                            <form id="supprimer-{{ $item->id }}" method="get" action="{{ route('itemDevis.edit' , ['itemDevi'=>$item->id]) }}"
                                >

                                @csrf
                                <button type="submit"
                                    class="text-black bg-gradient-to-r from-blue-400 via-cyan-300  to-cyan-200 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 shadow-md shadow-black dark:shadow-lg dark:shadow-blue-800/80 font-medium rounded-lg text-sm px-4 py-2.5 text-center transition  duration-500 ">
                                    Modifier
                                </button>
                            </form>
                        </td>
                        ...
                        <div class="relative p-4 w-full max-w-md max-h-full">
                            <div class="relative bg-zinc-100 rounded-lg shadow dark:bg-gray-700">
                                ...
                                <div class="p-4 md:p-5 text-center">
                                    ...
                                    <button id="{{ $item->id }}"
                                        data-modal-hide="popup-modal-{{ $item->id }}" type="submit" form="supprimer-{{ $item->id }}
                                        class="text-white bg-red-600 confirm-delete hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                        Oui
                                    </button>
                                    <button data-modal-hide="popup-modal-{{ $item->id }}" type="button"
                                        class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Annuler</button>
